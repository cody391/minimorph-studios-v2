      ...
      for (const project of liveProjects) {
        ...
        for (const day of MILESTONES) {
          ...
          const bodies: Record<number, string> = {
            1: `<p>Congratulations! Your MiniMorph website is live. Here are 3 quick things to do today:</p><ol><li>Share your new website URL on social media</li><li>Add it to your Google Business Profile</li><li>Send it to your existing customers</li></ol><p>Log in to your <a href="${ENV.appUrl || "https://www.minimorphstudios.net"}/portal">customer portal</a> to track visitors and request changes.</p>`,
            7: `<p>It's been one week since your site went live! Log in to your <a href="${ENV.appUrl || "https://www.minimorphstudios.net"}/portal">portal</a> to see your visitor stats and make any tweaks.</p><p>Remember: you have revision requests available — use them!</p>`,
            30: `<p>Your website has been live for 30 days. Log in to your <a href="${ENV.appUrl || "https://www.minimorphstudios.net"}/portal">portal</a> to see your monthly performance report including traffic, leads captured, and more.</p>`,
          };
      ...
      for (const project of liveProjects) {
        ...
        try {
          await sendEmail({
            ...
            html: `<!DOCTYPE html><html><body style="font-family:Inter,sans-serif;background:#111122;color:#eaeaf0;padding:24px;max-width:600px;margin:0 auto"><h2 style="color:#4a9eff">${seasonNames[quarter]} Website Tips</h2><p>Hi ${project.contactName || "there"},</p><p>Here are 3 things to update on your website this quarter:</p><ul>${tips.map((t: string) => `<li style="margin-bottom:8px">${t}</li>`).join("")}</ul><p><a href="${ENV.appUrl || "https://www.minimorphstudios.net"}/portal" style="color:#22c55e">Log in to request changes →</a></p><p style="color:#7a7a90;font-size:12px;margin-top:32px">MiniMorph Studios seasonal newsletter</p></body></html>`,
          });
          await database.insert(nurtureTable).values({
            customerId: project.customerId,
            type: "check_in",
      ...
      for (const project of liveProjects) {
        ...
        try {
          await sendEmail({
            ...
            html: `<!DOCTYPE html><html><body style="font-family:Inter,sans-serif;background:#111122;color:#eaeaf0;padding:24px;max-width:600px;margin:0 auto"><h2 style="color:#4a9eff">Take your website to the next level</h2><p>Hi ${project.contactName || "there"},</p><p>Your website is live and looking great. Here are some powerful add-ons that can help you get more customers:</p><ul><li style="margin-bottom:12px"><strong>Review Widget</strong> — Display your Google reviews automatically. Builds trust instantly.</li><li style="margin-bottom:12px"><strong>Booking Widget</strong> — Let customers schedule appointments directly from your website.</li><li style="margin-bottom:12px"><strong>AI Chat Bot</strong> — Answer customer questions 24/7 without lifting a finger.</li><li style="margin-bottom:12px"><strong>Lead Capture Form</strong> — Capture visitor info even when you're busy.</li></ul><p><a href="${ENV.appUrl || "https://www.minimorphstudios.net"}/portal" style="display:inline-block;background:#22c55e;color:#fff;padding:12px 24px;border-radius:999px;text-decoration:none;font-weight:600;margin-top:8px">Explore Add-ons →</a></p><p style="color:#7a7a90;font-size:12px;margin-top:32px">MiniMorph Studios</p></body></html>`,
          });
          await database.insert(nurtureTable).values({
            customerId: project.customerId,
            type: "upsell_attempt",
