...
const categoryIcons: Record<string, string> = {
  ...
  review_collector: "text-amber-600",
  lead_capture: "text-green-600",
  seo_autopilot: "text-terracotta",
  analytics_pro: "text-forest",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-serif text-forest">Widget & Add-On Catalog</h1>
        ...
        <Button onClick={() => setShowCreate(true)} className="bg-forest text-white hover:bg-forest/90 font-sans text-sm">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-serif text-forest">{widgets?.length ?? 0}</div>
              <div className="text-xs text-forest/50 font-sans">Total Widgets</div>
            </div>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-serif text-forest">{activeWidgets.length}</div>
              <div className="text-xs text-forest/50 font-sans">Active</div>
            </div>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-serif text-forest">${totalMRR.toLocaleString()}</div>
              <div className="text-xs text-forest/50 font-sans">Potential MRR/widget</div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          ...
          <div className="col-span-full text-center py-16">
            <Bot className="h-12 w-12 text-forest/15 mx-auto mb-4" />
            <h3 className="text-lg font-serif text-forest mb-2">No Widgets Yet</h3>
            <p className="text-sm text-forest/50 font-sans mb-4">Add AI widgets and add-ons to upsell to your customers.</p>
            <Button onClick={() => setShowCreate(true)} className="bg-terracotta text-white hover:bg-terracotta-light font-sans text-sm">
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <Bot className={`h-5 w-5 ${categoryIcons[widget.category] ?? "text-forest"}`} />
                    <h3 className="font-serif text-forest text-sm font-medium">{widget.name}</h3>
                  </div>
                  <Badge className={`text-[10px] font-sans ${widget.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                    {widget.isActive ? "Active" : "Inactive"}
                  </Badge>
                </div>
                <p className="text-xs text-forest/60 font-sans mb-3 line-clamp-2">{widget.description}</p>
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <span className="text-lg font-serif text-forest">${Number(widget.monthlyPrice).toLocaleString()}</span>
                    <span className="text-xs text-forest/50 font-sans">/mo</span>
                  </div>
                  {widget.setupFee && Number(widget.setupFee) > 0 && (
                    <span className="text-xs text-forest/50 font-sans">+ ${Number(widget.setupFee)} setup</span>
                  )}
                </div>
                <Badge className="text-[10px] font-sans bg-cream-dark/30 text-forest/60 mb-3">
                ...
                {widget.features && (
                  <div className="mt-2">
                    {(typeof widget.features === "string" ? widget.features.split(",") : []).slice(0, 3).map((f: string, i: number) => (
                      <span key={i} className="inline-block text-[10px] bg-sage/10 text-forest/60 rounded px-2 py-0.5 mr-1 mb-1 font-sans">{f.trim()}</span>
                    ))}
                  </div>
                )}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="font-serif text-forest">Add Widget to Catalog</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 font-sans">
            <div>
              <label className="text-xs text-forest/50 block mb-1">Widget Name</label>
              <Input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. AI Customer Support Bot" />
            </div>
            <div>
              <label className="text-xs text-forest/50 block mb-1">Category</label>
            ...
            <div>
              <label className="text-xs text-forest/50 block mb-1">Description</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-forest/50 block mb-1">Monthly Price ($)</label>
                <Input type="number" value={form.monthlyPrice} onChange={(e) => setForm(f => ({ ...f, monthlyPrice: e.target.value }))} placeholder="299" />
              </div>
              <div>
                <label className="text-xs text-forest/50 block mb-1">Setup Fee ($)</label>
                <Input type="number" value={form.setupFee} onChange={(e) => setForm(f => ({ ...f, setupFee: e.target.value }))} placeholder="0" />
              </div>
            </div>
            <div>
              <label className="text-xs text-forest/50 block mb-1">Features (comma-separated)</label>
          ...
          <DialogFooter>
            ...
            <Button
              className="bg-forest text-white hover:bg-forest/90 font-sans text-sm"
